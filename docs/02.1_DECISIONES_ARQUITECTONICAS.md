...
| [AD-41](#ad-41) | La categoría sin sexos no está restringida | ✅ Aceptada | Catálogo |
...
### La categoría sin sexos no está restringida
...
**Contexto.** `RN-83` agrega la relación Categoría ↔ Sexo (`category_genders`, `04` §9.2.16) para que el administrador decida en qué secciones de la navegación aparece cada categoría. Como toda relación N:M, admite un estado que hay que interpretar: **la categoría sin ninguna fila**. Ese estado no es hipotético — es el de **todas las categorías ya cargadas** en el momento de aplicar la migración.

**Alternativas.**

| Lectura de «sin filas» | Qué pasa al desplegar | Qué pasa al crear una categoría |
|---|---|---|
| **Sin restricción: aparece en todos** | El menú se ve **igual que antes**; nadie nota el despliegue | Nace visible; el administrador va quitando |
| Ninguno: no aparece en ningún lado | El menú **se vacía** salvo que la migración haga backfill de las 5 filas por categoría | Nace invisible; un olvido la esconde sin aviso |
| Prohibir el estado (`CHECK` de al menos una) | Imposible: ninguna restricción de esquema expresa «al menos una fila» sobre otra tabla | — |

**Decisión.** La ausencia de filas significa **sin restricción**: la categoría se ofrece en todos los ejes de sexo. La migración `c5b1f0a72e14` no hace backfill.

**Objetivos favorecidos.** `OA-08` y el despliegue sin sorpresas: una migración de esquema no debe cambiar lo que el cliente ve.

**Objetivos sacrificados.** El estado «esta categoría no va en ningún menú» no es expresable con este campo. Quien lo necesite tiene `is_active`, que ya existe y significa exactamente eso.

**Por qué el intercambio es aceptable.** Es el mismo criterio de `brands.show_in_strip` (`04` §9.2.3): el defecto reproduce el comportamiento anterior, de modo que la funcionalidad nace inerte y solo actúa cuando el administrador la usa. La alternativa obligaría a un backfill de datos comerciales dentro de una migración de esquema, que `04` §11.2 regla 3 desaconseja.

**Consecuencias.**
- El formulario del panel tilda los cinco sexos al crear una categoría: no cambia el resultado, pero le muestra al administrador que el campo existe y qué puede destildar.
- `appliesToGender` (`shared/utils/categoryGenders.js`) devuelve `true` con lista vacía, y esa es la línea que sostiene toda la compatibilidad hacia atrás.
- Una raíz que no aplica a un eje **sobrevive si alguna hija sí aplica**: `AD-29` hace que filtrar por el padre incluya a los descendientes, así que esconderla dejaría a esas hijas sin camino.

**Documentos afectados.** `01` `RN-83` · `04` §9.2.16 · `05_API.md` §7.5, §9.5 · `07_PANEL_ADMIN.md` · `09_COMPONENTES.md` §9.8
